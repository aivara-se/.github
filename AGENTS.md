...
`aivara-se/.github` is worked on by agents as much as by people, and this file is the instruction set every one of them reads first. It is a copy of the `aivara-se` agent convention, version `1`, adopted from `fdd819d751034bc7872dd25fbe29bbee161fc8a2`. Adopt it, do not fork it: repository-specific facts live in `.agents/config.yml` and in the sections below, and nothing else here is meant to be edited per repository.

The `aivara-se` organisation's profile and shared-convention repository: it holds the org README, the agent configuration every repository in the organisation adopts, and the programme reports.

## Current Project Focus

The agent-configuration rollout is the work in flight: every repository in `aivara-se` adopts `templates/agents-config/`. Do not restructure the template while those adoptions are under review.
...
- `README.md`: the organisation profile
