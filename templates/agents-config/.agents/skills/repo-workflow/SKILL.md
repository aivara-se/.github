...
- **Never** commit to `{{DEFAULT_BRANCH}}` directly, and **never** force-push a branch another agent or human has seen. Remote work is always a branch plus a pull request.
...
- The pull request body states what changed, what was verified (exact command and result), and what was deliberately left out. Request review from {{REVIEW_REQUEST_TARGETS}}.
